...
/// `DeferredReadOrOwn` writes through `WriteToAsyncRef` in both `Deferred` and `Own` states.
#[tokio::test]
...
mod derive_where_clause {
    //! Regression test: the future enums generated by `DirStructureAsync` must
    //! inherit the derived struct's `where` clause.
    //!
    //! Compiling this module is the test: without the struct's bounds being
    //! carried over into the generated code, the derive expansion fails with
    //! `<Vfs as VfsCore>::Path: MyTrait is not satisfied`.

    use dir_structure::DirStructureAsync;
    use dir_structure::traits::vfs::{PathType, VfsCore};
    use dir_structure_tools::dir_children::{DirChildren, Filter};

    trait MyTrait {}
    impl MyTrait for std::path::Path {}

    struct MyFilter;
    impl<P> Filter<P> for MyFilter
    where
        P: PathType + MyTrait + ?Sized,
    {
    ...
    struct MyFilterDir<Vfs>
    where
        Vfs: VfsCore,
        Vfs::Path: MyTrait,
    {
        #[dir_structure(path = self)]
        children: DirChildren<String, MyFilter, Vfs::Path>,
    }
}
